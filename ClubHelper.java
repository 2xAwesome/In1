...
public class ClubHelper {
    ...
    public void printMemberTeamOrder() {
		Logger.debugM();
		ArrayList<Member> members = cr.getMembers();
		Collections.sort(members, new MemberTeamComparator());

		System.out.println("Members ordered by their team:");
		String t = members.get(0).getTeam();
	...
	public void printMemberIdOrder() {
		Logger.debugM();
		ArrayList<Member> members = cr.getMembers();
		Collections.sort(members, new MemberIdComparator());
		
		System.out.println("Members ordered by id:");
		//String i = members.get(0).getId();
		for (Member m: members) {
			System.out.println(m);
		}
	}
	...
	/* public void printTeamParents(String team) { 
		Logger.debugM();
		ArrayList<Member> members = cr.getParent(team);
		Collections.sort(members, new MemberAlphaComparator());

		System.out.println("Parents in " + team + ":");
		for (Member m: members) {
	    	System.out.println(m);
		}
    } */
